...
import {getExercises, createLog} from "../api/workouts";
...
export default function AddLogModal({
  ...
  const sessionExercises = exercises.filter((ex) =>
    currentSessionExerciseIds.has(ex.id),
  );
  const otherExercises = exercises.filter(
    (ex) => !currentSessionExerciseIds.has(ex.id),
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="font-display" style={{fontSize: 24, marginBottom: 20}}>
          LOG A SET
        </div>
